...
import com.a0.kobalt.ui.buttons.KButtonRegistry
    ...
    private fun registerPrefixCommand(
        ...
        val commandMeta =
            CommandMeta(
                ...
                integrationTypes = annotation.integrationTypes,
                contextTypes = annotation.contextTypes,
                args = emptyList(),
...
// ////////////////////////////////////////////////  Command Handling  //////////////////////////////////////////////////
    ...
    internal fun handlePrefixCommand(event: MessageReceivedEvent) {
        ...
        if (command.requiredPermission != Permission.UNKNOWN &&
            ...
            event.channel.sendMessage(command.permissionDeniedMessage).queue()
            return
        }
    ...
    internal fun handleSlashCommand(event: SlashCommandInteractionEvent) {
        val command =
            slashCommandsMap[event.name.lowercase()]
                ?: throw CommandNotFound(commandName = event.name)

        if (command.requiredPermission != Permission.UNKNOWN &&
            event.member?.hasPermission(command.requiredPermission) == false
        ) {
            event.reply(command.permissionDeniedMessage).setEphemeral(true).queue()
            return
        }
